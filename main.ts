...
// Empujoncito final
sprites.onOverlap(SpriteKind.Player, SpriteKind.npc, function (sprite_player2, otherSprite) {
...
// Definimos la distancia de repulsión (ajustable)
    ...
    if (otherSprite == npc_controles) {
        ...
        intentar_comprar("shotgun", 50, assets.image`shotgun`, sprite_player2)
        intentar_comprar("rifle", 120, assets.image`rifle`, sprite_player2)
        intentar_comprar("Misterio", 9999, assets.image`easter_egg`, sprite_player2)
        sprite_player2.y += 10
    }
})
...
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.bullet_poryectile, function (sprite_proj22, otherSprite42) {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.normal_bullet, function (sprite_player, walking_enemy) {
    // ⭐️ COMPROBACIÓN DE INVULNERABILIDAD ⭐️
    if (dodge_roll == true) {
        return
    }
